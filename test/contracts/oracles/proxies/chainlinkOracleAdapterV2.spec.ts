...
contract('ChainlinkOracleAdapterV2', accounts => {
  const [
    deployerAccount,
  ] = accounts;
  ...
  let chainlinkOracleAdapter: ChainlinkOracleAdapterV2Contract;
  ...
  describe('#constructor', async () => {
    ...
    let subjectPriceMultiplier: BigNumber;

    beforeEach(async () => {
      subjectChainlinkOracleAddress = chainlinkAggregatorMock.address;
      subjectPriceMultiplier = new BigNumber(10);
    });

    async function subject(): Promise<ChainlinkOracleAdapterV2Contract> {
      return oracleHelper.deployChainlinkOracleAdapterV2Async(
        subjectChainlinkOracleAddress,
        subjectPriceMultiplier
      );
    }
  ...
  describe('#read', async () => {
    ...
    beforeEach(async () => {
      ...
      chainlinkOracleAdapter = await oracleHelper.deployChainlinkOracleAdapterV2Async(
