...
contract('LinkedListLibraryV2', accounts => {
  ...
  let linkedListLibraryMock: LinkedListLibraryMockV3Contract;
  ...
  beforeEach(async () => {
    ...
    linkedListLibraryMock = await libraryMockHelper.deployLinkedListLibraryMockV3Async();
  });
  ...
  describe('#readList', async () => {
    ...
    it('receives the correct array', async () => {
  ...
  describe('#getLatestValue', async () => {
    ...
    it('gets the latest logged value', async () => {
      const actualOutputArray = await subject();
      const expectedOutputArray = addedValues.slice(-1)[0];

      expect(actualOutputArray).to.be.bignumber.equal(expectedOutputArray);
    });
  });
});
